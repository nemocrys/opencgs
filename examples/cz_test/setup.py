...
def geometry(
    ...
    **kwargs,
    ...
    melt = cz.melt(
        model, dim, crucible, **config["melt"], crystal_radius=config["crystal"]["r"]
    )
    ...
    bnd_crystal_side = Shape(
        model, dim - 1, "bnd_crystal_side", crystal.get_interface(filling)
    )
    ...
    bnd_axtop_side = Shape(
        model, dim - 1, "bnd_axtop_side", ax_top.get_interface(filling)
    )
    bnd_axtop_bt = Shape(
        model,
        dim - 1,
    ...
    bnd_inductor_outside = Shape(
        model, dim - 1, "bnd_inductor_outside", inductor.get_interface(filling)
    )
    bnd_inductor_inside = Shape(
        model,
        dim - 1,
        "bnd_inductor_inside",
        inductor.get_interface(filling_in_inductor),
    )
...
def elmer_setup(
    ...
    **kwargs,
    ...
    # simulation
    sim = opencg.sim.ElmerSimulationCz(
        **config["general"],
        sim_dir=sim_dir,
