...
class Simulation:
    ...
    def _create_setup(self, visualize=False):  # TODO write metadata (package version opencgs, pyelmer, git-hashes,...)
    ...
    def _postprocessing_probes(self):  # TODO copied from sim-elmerthermo. Dublecheck!
        ...
        df = pd.read_table(self.sim_dir + '/results/probes.dat', names=header, sep=' ', skipinitialspace=True)
    ...
    def phase_interface(self):
        ...
        df = pd.read_table(f'{self.sim_dir}/results/phase-if.dat', names=header, sep= ' ',
...
class TransientSim(Simulation):
    def __init__(self, geo, geo_config, sim, sim_config, config_update={},
                 ...
                 l_end = 0.1, d_l = 3e-3, timesteps_per_dl=10, dt_out_factor=1, visualize=False,
                 **kwargs):
        ...
        if dt_out_factor != 1:
            raise NotImplementedError('The dt_out_factor option is not available yet :(')
    ...
    def execute(self):
        ...
        sim = SteadyStateSim(self.geo, geo_config, self.sim, sim_config,
                             sim_name='initialization', base_dir=self.sim_dir,
        ...
        while current_l < self.l_end:
            ...
            sim = TransientSubSim(old, t_end, self.geo, geo_config, self.sim,
                                    ...
                                    base_dir=self.sim_dir, visualize=self.visualize)
            sim.execute()
